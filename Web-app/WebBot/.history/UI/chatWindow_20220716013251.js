        ...
        function updateOnlineStatus() {
            ...
            chatWindow.prototype.bindSDKEvents = function (){
                ...
                me.bot.on("open", function (response) {
                    console.log('chat is active praveen----------------');
                    me.onBotReady();
                });

                me.bot.on("message", function (message) {
                    console.log('chat is message passing praveen----------------', message);
            ...
            chatWindow.prototype.sendMessage = function (chatInput, renderMsg,msgObject) {
                var me = this;
                if (chatInput.val().trim() === "" && $('.attachment').html().trim().length == 0) {
                    return;
                }
                ...
                if (Object.keys(me.attachmentInfo).length > 0 && chatInput.val().trim().length) {
                    me.attachmentInfo.fileId = attachmentInfo.fileId;
                    messageToBot["message"] = { body: chatInput.val().trim(), attachments: [me.attachmentInfo] };
                } else if (Object.keys(me.attachmentInfo).length > 0) {
                    me.attachmentInfo.fileId = attachmentInfo.fileId;
                    messageToBot["message"] = { attachments: [me.attachmentInfo] };
                }
                else {
                    messageToBot["message"] = { body: chatInput.val().trim() };
                }
            ...
            chatWindow.prototype.getChatTemplate = function (tempType) {
                ...
                '<div class="footerContainer pos-relative"> \
                    {{if userAgentIE}} \
                    <textarea role="textbox" class="chatInputBox inputCursor" aria-label="Message" aria-label="Message" contenteditable="true" placeholder="${botMessages.message}"></textarea> \
                    {{else}} \
                    <textarea role="textbox" class="chatInputBox" aria-label="Say this word" contenteditable="true" placeholder="${botMessages.message}"></textarea> \
                    {{/if}} \
                        ...
                        {{each(key, msgItem) msgData.message}} \
                            {{if msgItem.cInfo && msgItem.type === "text"}} \
                                <li data-time="${msgData.createdOnTimemillis}" id="${msgData.messageId || msgItem.clientMessageId}"\
                                    ...
                                    {{if msgData.createdOn}}<div aria-hidden="true" aria-live="off" class="extra-info">{{if msgData.type == "bot_response"}}Bot{{else}}You{{/if}}</div>{{/if}} \
                    ...
                    {{each(key, msgItem) msgData.message}} \
                        {{if msgItem.component && msgItem.component.payload.url}} \
                            <li data-time="${msgData.createdOnTimemillis}" id="${msgData.messageId || msgItem.clientMessageId}"\
                                ...
                                {{if msgData.createdOn}}<div class="extra-info">{{if msgData.type == "bot_response"}}Bot{{else}}User{{/if}}</div>{{/if}} \
                ...
                {{if msgData.message}} \
                    <li data-time="${msgData.createdOnTimemillis}" id="${msgData.messageId || msgItem.clientMessageId}"\
                        ...
                        <div class="buttonTmplContent"> \
                            {{if msgData.createdOn}}<div aria-live="off" class="extra-info">{{if msgData.type == "bot_response"}}Bot{{else}}User{{/if}}</div>{{/if}} \
