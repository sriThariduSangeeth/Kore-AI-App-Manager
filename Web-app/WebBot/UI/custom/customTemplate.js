(function($){
	...
	customTemplate.prototype.renderMessage = function (msgData) {
		...
		}else if (msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "customButton") {
			messageHtml = $(this.getChatTemplate("customButtonTemplate")).tmpl({
				'msgData': msgData,
				'helpers': this.helpers,
				'extension': this.extension
			});
	...
	*/
	...
	customTemplate.prototype.getChatTemplate = function (tempType) {
		...
		*/
		var customButtonTemplate11 = '<script id="chat_message_tmpl" type="text/x-jqury-tmpl"> \
                {{if msgData.message}} \
		...
		/* Sample template structure for multi-select checkboxes
			...
			print(JSON.stringify(message)); 
		*/
		var customButtonTemplate = '<script id="chat_message_tmpl" type="text/x-jqury-tmpl"> \
			{{if msgData.message}} \
			<li {{if msgData.type !== "bot_response"}}id="msg_${msgItem.clientMessageId}"{{/if}} class="{{if msgData.type === "bot_response"}}fromOtherUsers{{else}}fromCurrentUser{{/if}} with-icon"> \
...
print(JSON.stringify(message)); */
	...
	{{/if}} \
		...
		if (tempType === "dropdown_template") {
			...
			return listWidget;
		}else if(tempType === "customButtonTemplate"){
